...
#define TEXT_BUFFER_HEIGHT_MAX 20//10
#define TEXT_BUFFER_LINE_LENGTH_MAX 120//60
...
boolean tb_display_word_wrap = false;//true;
...
void tb_display_init(int ScreenRotation){
  ...
  switch (ScreenRotation) {
    ...
    case 2: case 4: {
      // 10 rows of text in portrait mode
      text_buffer_height = 20;//10;
      text_buffer_line_length = 60;//30;
...
void tb_display_print_char(byte data){
  ...
  if (data == '\b') {
    if(screen_xpos > 0) {
      ...
      text_buffer[text_buffer_write_pointer_y][text_buffer_write_pointer_x] = '\0';
      M5.Lcd.drawChar(' ',screen_xpos,screen_ypos,TEXT_SIZE);
    }
  }
